...
FLUENTD_VERSION=1.6.0
PATCH_VERSION=$$(($(shell curl -sS "https://hub.docker.com/v2/repositories/${DOCKER_IMAGE}/tags/?page_size=1&page=1&name=${FLUENTD_VERSION}-&ordering=last_updated" | jq -r '.results[0].name' | cut -f2 -d '-') + 1))
...
release:
	git tag "${FLUENTD_VERSION}-${PATCH_VERSION}"
	git push --tags
build: lint
